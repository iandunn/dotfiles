...
The metacharacter guard tracks quote state rather than scanning the raw line, because bash does. `;`, `&`, `|`, `(`, `)`, `<`, `>`, and a newline prompt only outside quotes, since bash treats each of them literally inside quotes of either kind and none can start a second command from there. `$` and a backtick prompt wherever they sit, expansion inside double quotes being real, and so does a backslash followed by a newline, because bash splices the next line onto the command before parsing it. That's what lets a SQL subquery's parens through while `wp option get x && wp db reset --yes` still prompts.

A redirect whose target is `/dev/null` (`2>/dev/null`, `>>/dev/null`, `&>/dev/null`, `< /dev/null`) or a file descriptor (`2>&1`) is the one redirect that doesn't prompt, since it throws output away and can't write a real file or chain anything. `> /tmp/x`, or a target that only looks like it (`/dev/null.bak`, `>&somefile`), still prompts. On the Bash path these are stripped from the line before it's tokenized, so a trailing `2>/dev/null` no longer counts as a `db query` second statement; the MCP path has no shell, so nothing is stripped there.
