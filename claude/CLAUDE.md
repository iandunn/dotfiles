...
Scratch cleanup under `.claude/tmp/` is handled two different ways, and the split is deliberate. A *relative* operand makes the hook stay silent, leaving the decision to settings.json's `rm .claude/tmp/*` and `mv * .claude/tmp/*` allow rules. An *absolute* one is allowed by the hook itself, because those rules are literal prefixes that an absolute path can never match, so silence would drop the command to the auto-mode classifier with no guaranteed approval -- and `running-commands.md` tells you to write absolute paths, so that is the spelling this actually arrives in. The absolute form is scoped to the working directory's own `.claude/tmp/`, not any path containing those segments, so sweeping into another project's scratch still prompts. Both forms grant the same thing; only the layer deciding differs.
