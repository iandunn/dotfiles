...
- Put temporary files in `.claude/tmp/` inside the project, not the system `/tmp`. A permanent artifact like PDF -> text goes in the project's `_notes` folder if one exists, or the Relay folder when the project uses Relay. If neither exists, ask where it should go instead of picking a spot.
