...
# LocalWP shells
alias shcdcr='sh     "$HOME/dotfiles/localwp/ssh-entry/vBmBenAQS.sh"'
...
alias gorramit_firefox='rm -f ~/Library/Application Support/Firefox/Profiles/**/.parentlock && pkill -9 -u "$(whoami)" -f "org.mozilla.updater" &&   sudo pkill -9 -f "/Library/PrivilegedHelperTools/org.mozilla.updater" &&   rm -rf "$HOME/Library/Caches/Mozilla/updates/Applications/Firefox Developer Edition"'
